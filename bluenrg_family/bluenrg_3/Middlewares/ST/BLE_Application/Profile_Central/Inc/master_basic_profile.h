/**
  ...
  * @brief   Header file for Bluetooth LE master/central library APIs.
  ******************************************************************************
  * @attention
  *
 ...
 */
#define LIMITED_DISCOVERY_PROCEDURE 0x00

/**
 * @brief General discovery procedure used during device discovery.
 * Define used in Master_DeviceDiscovery()
 */
#define GENERAL_DISCOVERY_PROCEDURE 0x01
 ...
 * Define used in void Master_PeerDataEvent_CB
 */
#define WRITE_DATA_RECEIVED 0x0C
 ...
 * Define used in void Master_PeerDataEvent_CB
 */
#define READ_DATA_RECEIVED 0x0D
...
/**
 * @brief This function allows the master to send a response to read or write event
 *        (only if Resp_needed==1) from the peer device.
 * @param Connection_handle Connection handle that identifies the connection.
 * @param Attr_Handle Attribute handle for wich the response command is issued.
 ...
 * @note This function must be called inside the Master_PeerDataEvent_CB() callback
 */
uint8_t Master_Response(uint16_t Connection_handle,
                        uint16_t Attr_Handle,
                        uint8_t Error_Code,
                        uint16_t Val_Length,
                        uint8_t Val[]);
...
/**
 * @brief  This callback will be called when a gatt_srv_write_event or gatt_srv_read_event is generated 
 * and the application has to send ACI_GATT_SERV_RESP command (for write only if Resp_Needed == 1)
 * @param procedure Master procedure to serve:\n
 * - @ref READ_DATA_RECEIVED\n
 * - @ref WRITE_DATA_RECEIVED\n
 * @param connection_handle Connection handle
 * @param data Characteristic information to allow the read or write operation. (see @ref dataPermitExchangeType)
 * and attribute modifications.
 */
extern void Master_PeerDataEvent_CB(uint8_t *procedure, uint16_t *connection_handle, 
                                    uint8_t *Resp_Needed, dataPermitExchangeType  *data); 
...
/**
 * @brief This function allows to set the scan configuration parameters (BLE stack v3.x)
 * @retval Status of the call
 */
uint8_t Master_DeviceScanConfiguration(devDiscoveryType *param); 

/**
 * @brief This function allows to set the connection configuration parameters (BLE stack v3.x)
 * @retval Status of the call
 */
uint8_t Master_DeviceConnectionConfiguration(connDevType *param);

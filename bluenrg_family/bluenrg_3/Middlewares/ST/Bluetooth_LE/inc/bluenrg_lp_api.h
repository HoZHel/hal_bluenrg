 ...
 *        given during pairing, the command returns  BLE_STATUS_NOT_ALLOWED. It
 *        is recommended to force the use of Secure Connections for pairing by
 *        using Secure Connections Only Mode, since it is the only way to
 *        protect the pairing from passive eavesdropping.
...
tBleStatus aci_gatt_clt_read_multiple_var_len_char_value(uint16_t Connection_Handle,
                                                         ...
                                                         Handle_Entry_t Handle_Entry[]);
/**
 * @brief Adds the GATT service to the database and enables EATT.
 * @retval Value indicating success or error code.
 */
 ...
 *        request the server to prepare to write the value of an attribute.  The
 *        responses of the procedure are given through the @ref
 *        aci_eatt_clt_prepare_write_resp_event event.  The end of the procedure
 *        is indicated by a @ref aci_gatt_eatt_clt_proc_complete_event.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 ...
 *        values currently held in the prepare queue from this client.  The
 *        result of the procedure is given through the @ref
 *        aci_eatt_clt_exec_write_resp_event event.  The end of the procedure is
 *        indicated by a @ref aci_gatt_eatt_clt_proc_complete_event event.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 * @param CID Local endpoint identifying the EATT channel.
 ...
 * @brief Starts the GATT client procedure to discover all primary services on
 *        the server. The responses of the procedure are given through the @ref
 *        aci_eatt_clt_read_by_group_type_resp_event event.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 * @param CID Local endpoint identifying the EATT channel.
 ...
 * @brief Starts the procedure to discover the primary services of the specified
 *        UUID on the server. The responses of the procedure are given through
 *        the @ref aci_eatt_clt_find_by_type_value_resp_event event. The end of
 *        the procedure is indicated by a @ref
 *        aci_gatt_eatt_clt_proc_complete_event event.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 * @param CID Local endpoint identifying the EATT channel.
 ...
 * @brief Starts the procedure to find all included services. The responses of
 *        the procedure are given through the @ref
 *        aci_eatt_clt_read_by_type_resp_event event. The end of the procedure
 *        is indicated by a @ref aci_gatt_eatt_clt_proc_complete_event event.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 * @param CID Local endpoint identifying the EATT channel.
 ...
 * @brief Starts the procedure to discover all the characteristics of a given
 *        service. When the procedure is completed, a @ref
 *        aci_gatt_eatt_clt_proc_complete_event event is generated. Before
 *        procedure completion the response packets are given through @ref
 *        aci_eatt_clt_read_by_type_resp_event event.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 * @param CID Local endpoint identifying the EATT channel.
 ...
 *        aci_gatt_eatt_clt_proc_complete_event event is generated. Before
 *        procedure completion the response packets are given through @ref
 *        aci_gatt_eatt_clt_disc_read_char_by_uuid_resp_event event.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 * @param CID Local endpoint identifying the EATT channel.
 ...
 *        aci_gatt_eatt_clt_proc_complete_event event is generated. Before
 *        procedure completion the response packets are given through @ref
 *        aci_eatt_clt_find_info_resp_event event.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 * @param CID Local endpoint identifying the EATT channel.
...
/**
 * @brief Starts the procedure to read an attribute value. When the procedure is
 *        completed, a @ref aci_gatt_eatt_clt_proc_complete_event event is
 *        generated. Before procedure completion the response packet is given
 *        through @ref aci_eatt_clt_read_resp_event event.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 * @param CID Local endpoint identifying the EATT channel.
 ...
 *        through @ref aci_eatt_clt_read_blob_resp_event event.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 * @param CID Local endpoint identifying the EATT channel.
 ...
 *        Handles. Only values that have a known fixed size can be read, with
 *        the exception of the last value that can have a variable length. When
 *        the procedure is completed, a @ref
 *        aci_gatt_eatt_clt_proc_complete_event event is generated. Before
 *        procedure completion the response packets are given through @ref
 *        aci_eatt_clt_read_multiple_resp_event event. The response only
 *        contains a set of Characteristic Values that is less than or equal to
 *        (ATT_MTU - 1) octets in length. If the Set Of Values is greater than
 *        (ATT_MTU - 1) octets in length, only the first (ATT_MTU - 1) octets
 ...
 *        for any response from the server. No events are generated after this
 *        command is executed. Writing attributes using this function is not
 *        considered reliable by the standard: the peer will discard commands
 *        that are received but cannot be processed.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 * @param CID Local endpoint identifying the EATT channel.
...
tBleStatus aci_gatt_eatt_srv_notify(uint16_t Connection_Handle,
                                    ...
                                    uint8_t Val[]);
/**
 * @brief Notify multiple characteristic values to a client, using Enhanced ATT.
 * @param Connection_Handle Connection handle for which the attribute value will
 *        be read.
 * @param CID Local endpoint identifying the EATT channel.
 * @param Flags Reserved for future use. Its value must be set to 0.
 ...
 *        a server when the client knows the Characteristic Value Handles. This
 *        procedure is useful when the attributes to read have a variable or
 *        unknown value length (otherwise
 *        aci_gatt_eatt_clt_read_multiple_char_value may be used). When the
 *        procedure is completed, a @ref aci_gatt_eatt_clt_proc_complete_event
 *        event is generated. Before procedure completion the response packets
 *        are given through @ref aci_eatt_clt_read_multiple_var_len_resp_event
 *        event.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
...
tBleStatus aci_gatt_srv_read_multiple_instance_handle_value(uint16_t Connection_Handle,
                                                            ...
                                                            uint8_t * *Value);
/**
 * @brief Command to be given in response to aci_gatt_eatt_srv_read_event,
 *        aci_gatt_eatt_srv_write_event, aci_eatt_srv_prepare_write_req_event or
 *        aci_eatt_srv_exec_write_req_event. It ends the ATT transaction
 *        initiated by the remote client.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 ...
 * @param Val_Length Length of the Val field.
 * @param Val Pointer to the value that must be returned in the response, in
 *        case this is a reply to an aci_gatt_eatt_srv_read_event(). In other
 *        cases it is ignored.
 * @retval Value indicating success or error code.
 */
 ...
 * @brief Starts the procedure to write an attribute (characteristic value or
 *        descriptor). When the procedure is completed, a @ref
 *        aci_gatt_eatt_clt_proc_complete_event event is generated.  Note: the
 *        buffer containing the value to be written must be kept valid until the
 *        @ref aci_gatt_eatt_clt_proc_complete_event is received
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
 * @param CID Local endpoint identifying the EATT channel.
 ...
 *        aci_gatt_eatt_clt_proc_complete_event event is emitted indicating that
 *        the procedure is completed, or an error was received. During the
 *        procedure, aci_eatt_clt_prepare_write_resp_event and
 *        aci_eatt_clt_exec_write_resp_event events are raised.  Note: the
 *        buffer containing the value to be written must be kept valid until the
 *        @ref aci_gatt_eatt_clt_proc_complete_event is received.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 ...
 * @brief Starts the procedure to write a characteristic reliably (a check is
 *        made on the written values). When the procedure is completed, a  @ref
 *        aci_gatt_eatt_clt_proc_complete_event event is generated. During the
 *        procedure, @ref aci_eatt_clt_prepare_write_resp_event and @ref
 *        aci_eatt_clt_exec_write_resp_event events are raised.  Note: the
 *        buffer containing the value to be written must be kept valid until the
 *        @ref aci_gatt_eatt_clt_proc_complete_event is received.
 * @param Connection_Handle Connection handle that identifies the connection.

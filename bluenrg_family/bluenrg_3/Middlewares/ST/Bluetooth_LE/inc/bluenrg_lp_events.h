...
/**
 * @brief Event used to receive asynchronous data from the controller on a link.
 *
 * @note The event is available only when "Link Layer only" mode is active.
 *
...
tBleStatus hci_rx_acl_data_event(uint16_t Connection_Handle,
                                 ...
                                 uint8_t*  PDU_Data);
/**
 * @brief Callback used to receive isochronous data from the Controller.
 *
 * @param      Connection_Handle    Connection handle. Range: 0x0000-0x0EFF (0x0F00 - 0x0FFF Reserved for future use)
 * @param      PB_Flag              Packet boundary flag
 ...
 * @return  Error code
 */
tBleStatus hci_le_rx_iso_data_event(uint16_t Connection_Handle,
                                    uint8_t  PB_Flag,
                                    uint8_t TS_Flag,
 ...
 *@brief Standard HCI LE Meta events.
 *@{
 */
/* HCI LE meta events */
...
typedef struct Connection_Handle_t_s {
 ...
 *        Request). After this event is received, aci_gatt_eatt_srv_resp() must
 *        be used to send the response. This event is not generated if the read
 *        is requested on a characteristic or a descriptor which have an
 *        associated buffer handled by the stack (see ble_gatt_chr_def_t and
 ...
 *        needed, application must respond with an aci_gatt_eatt_srv_resp().
...
void aci_gatt_eatt_srv_write_event(uint16_t Connection_Handle,
                                   ...
                                   uint8_t Data[]);
/**
 * @brief This event is generated when a prepare write request is received on an
 *        Enhanced ATT bearer. Application should queue this request and execute
 *        or discard it only when a aci_eatt_srv_exec_write_req_event is
 *        received.
 * @param Connection_Handle Handle identifying the connection where the prepare
 *        write operation has been received.
 * @param CID Local endpoint identifying the EATT channel.
...
typedef struct local_reconf_cid_t_s {
  /** Array of up to 5 two-octet values that represent the local channel endpoints
 ...
 *@brief Events for HAL and proprietary LL.
 *@{
 */
/* ACI HAL events */
/**

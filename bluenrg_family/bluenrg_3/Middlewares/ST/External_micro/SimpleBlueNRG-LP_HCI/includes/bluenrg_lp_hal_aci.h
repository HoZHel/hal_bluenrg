 ...
 *             - 0x00000004: CONTROLLER_MASTER_ENABLED
 ...
 *        aci_hal_tone_stop command. If antenna switch capability is supported,
 *        the default antenna ID is used.
 ...
 *        communication). Any GPIO not selected with the Antenna_IDs mask is not
 *        configured. A system reset is needed to restore GPIOs state to its
 ...
 *        zero to left shift the ANTENNA_ID signal by the given number of bits.
 *        This number does not affect the pin selected by Antenna_IDs parameter.
 *        E.g. to have the ANTENNA_ID signal output on PB[2:4], set Antenna_IDs
 *        to enable ANTENNA_ID[2:4] (Antenna_IDs = 0x1C) and Antenna_ID_Shift to
 *        2.
 *        Values:
 *        - 0 ... 6

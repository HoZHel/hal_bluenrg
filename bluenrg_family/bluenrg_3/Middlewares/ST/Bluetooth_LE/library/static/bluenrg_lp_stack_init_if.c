...
WEAK_FUNCTION(void aci_eatt_clt_read_multiple_var_len_resp_event(uint16_t Connection_Handle,
                                                   ...
                                                   uint16_t Event_Data_Length,
                                                   uint8_t Set_Of_Values[]){});
WEAK_FUNCTION(void aci_gatt_eatt_srv_read_event(uint16_t Connection_Handle,
                                  uint16_t CID,
                                  uint16_t Attribute_Handle,
                                  uint16_t Data_Offset){});
WEAK_FUNCTION(void aci_gatt_eatt_srv_write_event(uint16_t Connection_Handle,
                                   uint16_t CID,
                                   uint8_t Resp_Needed,
                                   uint16_t Attribute_Handle,
                                   uint16_t Data_Length,
                                   uint8_t Data[]){});
WEAK_FUNCTION(void aci_eatt_srv_prepare_write_req_event(uint16_t Connection_Handle,
                                          uint16_t CID,
                                          uint16_t Attribute_Handle,
                                          uint16_t Data_Offset,
                                          uint16_t Data_Length,
...
WEAK_FUNCTION(void hci_le_big_sync_lost_event(uint8_t BIG_Handle,
                                uint8_t Reason){});
WEAK_FUNCTION(void hci_le_request_peer_sca_complete(uint8_t Status,
                                      uint16_t Connection_Handle,
...
tBleStatus BLE_STACK_Init(const BLE_STACK_InitTypeDef *BLE_STACK_InitStruct)
{
  ...
  REGISTER_CALLBACK(ACI_L2CAP_COS_CONNECTION_EVENT, aci_l2cap_cos_connection_event);
  REGISTER_CALLBACK(ACI_L2CAP_ECFC_RECONFIGURATION_EVENT, aci_l2cap_ecfc_reconfiguration_event);
  ...
  REGISTER_CALLBACK(ACI_ATT_CLT_READ_MULTIPLE_VAR_LEN_RESP_EVENT, aci_att_clt_read_multiple_var_len_resp_event);
  REGISTER_CALLBACK(ACI_GATT_EATT_SRV_ATTRIBUTE_MODIFIED_EVENT, aci_gatt_eatt_srv_attribute_modified_event);
  REGISTER_CALLBACK(ACI_GATT_EATT_PROC_TIMEOUT_EVENT, aci_gatt_eatt_proc_timeout_event);
  REGISTER_CALLBACK(ACI_EATT_CLT_FIND_INFO_RESP_EVENT, aci_eatt_clt_find_info_resp_event);
  REGISTER_CALLBACK(ACI_EATT_CLT_FIND_BY_TYPE_VALUE_RESP_EVENT, aci_eatt_clt_find_by_type_value_resp_event);
  REGISTER_CALLBACK(ACI_EATT_CLT_READ_BY_TYPE_RESP_EVENT, aci_eatt_clt_read_by_type_resp_event);
  REGISTER_CALLBACK(ACI_EATT_CLT_READ_RESP_EVENT, aci_eatt_clt_read_resp_event);
  REGISTER_CALLBACK(ACI_EATT_CLT_READ_BLOB_RESP_EVENT, aci_eatt_clt_read_blob_resp_event);
  REGISTER_CALLBACK(ACI_EATT_CLT_READ_MULTIPLE_RESP_EVENT, aci_eatt_clt_read_multiple_resp_event);
  REGISTER_CALLBACK(ACI_EATT_CLT_READ_BY_GROUP_TYPE_RESP_EVENT, aci_eatt_clt_read_by_group_type_resp_event);
  REGISTER_CALLBACK(ACI_EATT_CLT_PREPARE_WRITE_RESP_EVENT, aci_eatt_clt_prepare_write_resp_event);
  REGISTER_CALLBACK(ACI_EATT_CLT_EXEC_WRITE_RESP_EVENT, aci_eatt_clt_exec_write_resp_event);
  REGISTER_CALLBACK(ACI_GATT_EATT_CLT_INDICATION_EVENT, aci_gatt_eatt_clt_indication_event);
  REGISTER_CALLBACK(ACI_GATT_EATT_CLT_NOTIFICATION_EVENT, aci_gatt_eatt_clt_notification_event);
  REGISTER_CALLBACK(ACI_GATT_EATT_CLT_PROC_COMPLETE_EVENT, aci_gatt_eatt_clt_proc_complete_event);
  REGISTER_CALLBACK(ACI_GATT_EATT_CLT_ERROR_RESP_EVENT, aci_gatt_eatt_clt_error_resp_event);
  REGISTER_CALLBACK(ACI_GATT_EATT_CLT_DISC_READ_CHAR_BY_UUID_RESP_EVENT, aci_gatt_eatt_clt_disc_read_char_by_uuid_resp_event);
  REGISTER_CALLBACK(ACI_GATT_EATT_SRV_CONFIRMATION_EVENT, aci_gatt_eatt_srv_confirmation_event);
  REGISTER_CALLBACK(ACI_EATT_SRV_EXEC_WRITE_REQ_EVENT, aci_eatt_srv_exec_write_req_event);
  REGISTER_CALLBACK(ACI_EATT_CLT_READ_MULTIPLE_VAR_LEN_RESP_EVENT, aci_eatt_clt_read_multiple_var_len_resp_event);
  REGISTER_CALLBACK(ACI_GATT_EATT_SRV_READ_EVENT, aci_gatt_eatt_srv_read_event);
  REGISTER_CALLBACK(ACI_GATT_EATT_SRV_WRITE_EVENT, aci_gatt_eatt_srv_write_event);
  REGISTER_CALLBACK(ACI_EATT_SRV_PREPARE_WRITE_REQ_EVENT, aci_eatt_srv_prepare_write_req_event);
  ...
  REGISTER_CALLBACK(HCI_LE_CIS_ESTABLISHED_EVENT, hci_le_cis_established_event);
  REGISTER_CALLBACK(HCI_LE_CIS_REQUEST_EVENT, hci_le_cis_request_event);
  REGISTER_CALLBACK(HCI_LE_CREATE_BIG_COMPLETE_EVENT, hci_le_create_big_complete_event);
  REGISTER_CALLBACK(HCI_LE_TERMINATE_BIG_COMPLETE_EVENT, hci_le_terminate_big_complete_event);
  REGISTER_CALLBACK(HCI_LE_BIG_SYNC_ESTABLISHED_EVENT, hci_le_big_sync_established_event);
  REGISTER_CALLBACK(HCI_LE_BIG_SYNC_LOST_EVENT, hci_le_big_sync_lost_event);
  REGISTER_CALLBACK(HCI_LE_REQUEST_PEER_SCA_COMPLETE, hci_le_request_peer_sca_complete);
  ...
  REGISTER_CALLBACK(HCI_LE_BIGINFO_ADVERTISING_REPORT_EVENT, hci_le_biginfo_advertising_report_event);
  REGISTER_CALLBACK(HCI_LE_SUBRATE_CHANGE_EVENT, hci_le_subrate_change_event);

                                         ...
                                         cid_t cid[])
{
  struct hci_request rq;
  uint8_t cmd_buffer[532];
  aci_l2cap_ecfc_connection_req_cp0 *cp0 = (aci_l2cap_ecfc_connection_req_cp0*)(cmd_buffer);
  tBleStatus status = 0;
  uint8_t index_input = 0;
  cp0->Connection_Handle = htob(Connection_Handle, 2);
  index_input += 2;
  ...
  cp0->MPS = htob(MPS, 2);
  index_input += 2;
  cp0->CID_Count = htob(CID_Count, 1);
  index_input += 1;
  /* var_len_data input */
  {
    for (int i = 0; i < CID_Count; i++) {
      packed_cid_t *cid_tmp = (packed_cid_t *)(((uint8_t *)cmd_buffer) + index_input);
      cid_tmp->Local_CID = cid[i].Local_CID;
      index_input += 2;
    }
  }
  Osal_MemSet(&rq, 0, sizeof(rq));
                                          ...
                                          cid_t cid[])
{
  struct hci_request rq;
  uint8_t cmd_buffer[532];
  aci_l2cap_ecfc_connection_resp_cp0 *cp0 = (aci_l2cap_ecfc_connection_resp_cp0*)(cmd_buffer);
  tBleStatus status = 0;
  uint8_t index_input = 0;
  cp0->Connection_Handle = htob(Connection_Handle, 2);
  index_input += 2;
                                          ...
                                          cid_t cid[])
{
  struct hci_request rq;
  uint8_t cmd_buffer[532];
  aci_l2cap_ecfc_reconfigure_req_cp0 *cp0 = (aci_l2cap_ecfc_reconfigure_req_cp0*)(cmd_buffer);
  tBleStatus status = 0;
  uint8_t index_input = 0;
  cp0->Connection_Handle = htob(Connection_Handle, 2);
  index_input += 2;
                                           ...
                                           uint16_t Result)
{
  struct hci_request rq;
  uint8_t cmd_buffer[532];
  aci_l2cap_ecfc_reconfigure_resp_cp0 *cp0 = (aci_l2cap_ecfc_reconfigure_resp_cp0*)(cmd_buffer);
  tBleStatus status = 0;
  uint8_t index_input = 0;
  cp0->Connection_Handle = htob(Connection_Handle, 2);
  index_input += 2;
